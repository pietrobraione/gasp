...
public class CrossoverFunctionPrefix<T extends Gene<T>> implements CrossoverFunction<T> {
	...
	public Pair<List<T>> doCrossover(long seed, List<T> chromosome1, List<T> chromosome2) {
		...
		final List<T> chromosomeCombined1 = combineChromosomes(chromosome1.subList(0, cutPoint11), (random.nextBoolean() ? chromosome2.subList(cutPoint21, cutPoint22) : chromosome2.subList(cutPoint22, chromosome2.size())));
		final List<T> chromosomeCombined2 = combineChromosomes(chromosome2.subList(0, cutPoint21), (random.nextBoolean() ? chromosome1.subList(cutPoint11, cutPoint12) : chromosome1.subList(cutPoint12, chromosome1.size())));
